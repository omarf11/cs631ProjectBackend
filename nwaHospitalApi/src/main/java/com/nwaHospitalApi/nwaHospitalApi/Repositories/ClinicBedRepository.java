...
public interface ClinicBedRepository extends JpaRepository<clinicBed, Integer> {
    ...
    int countByBedIdAndPatientId(Integer bedId, Integer patientId);

    @Modifying
    @Transactional
    @Query(value  = "UPDATE clinic_bed c SET c.patient_id = :patientId WHERE c.bed_id = :bedId" , nativeQuery = true)
    void updatePatientIdByBedId(Integer bedId, Integer patientId);

    @Modifying
    @Transactional
    @Query(value = "UPDATE clinic_bed c SET c.patient_id = null WHERE c.bed_id = :bedId" , nativeQuery = true)
    void updatePatientIdToNullByBedId(Integer bedId);
}
